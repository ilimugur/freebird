...
public class AcrobacyEventHandler : MonoBehaviour
{
	...
	public void OnCompleteLoop(AcrobacyEvent ev)
	{
		if (_lastLoopEvent != null && Vector3.Distance(ev.Position, _lastLoopEvent.Position) <= LoopEventComboDistance)
		{
			...
			EventManager.Instance.TriggerEvent(Constants.EVENT_INCREMENT_SCORE, _loopEventComboCount * 2);
			EventManager.Instance.TriggerEvent(Constants.EVENT_GAIN_FUEL, _loopEventComboCount * 2);
		}
		else
		{
	...
	public void OnEndVerticalStance(AcrobacyEvent ev)
	{
		if (_verticalStanceStartEvent != null)
		{
			...
			if (time >= VerticalFlightTimeRequirement)
			{
				...
				EventManager.Instance.TriggerEvent(Constants.EVENT_INCREMENT_SCORE, 10 + intTime);
				EventManager.Instance.TriggerEvent(Constants.EVENT_GAIN_FUEL, 10 + intTime);
			}
